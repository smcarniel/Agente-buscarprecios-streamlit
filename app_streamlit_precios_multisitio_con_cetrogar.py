...
# Función general multisitio
def buscar_precios_multisitio(producto, cantidad):
    resultados = []

    precios_ml = buscar_precios_mercadolibre(producto, cantidad)
    resultados.extend(precios_ml)

    precios_cetrogar = buscar_precios_cetrogar(producto, cantidad)
    resultados.extend(precios_cetrogar)
...
def main():
    ...
    st.write("Subí un archivo Excel con los productos en la columna B y buscá los mejores precios en múltiples sitios (MercadoLibre + Cetrogar).")
    ...
    if archivo is not None:
        ...
        if st.button("🚀 Buscar precios en múltiples sitios"):
            resultados = []
            ...
            with st.spinner("🔍 Buscando precios, por favor esperá..."):
                for idx, producto in enumerate(productos, start=1):
                    precios = buscar_precios_multisitio(producto, cantidad_precios)
                    if precios:
            ...
            st.download_button(
                ...
                data=output,
                file_name="precios_buscados_multisitio_con_cetrogar.xlsx",
